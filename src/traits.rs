...
use crate::{dom::RefFn, EventOptions};
...
#[derive(Debug)]
pub(crate) struct MapMultiStr<A, F>
where
    F: Fn(&str) -> String,
{
    multi_str: A,
...
where
    ...
    T: Fn(&str) -> String,
{
    fn find_map<A, F>(&self, mut f: F) -> Option<A>
    where
        F: FnMut(&str) -> Option<A>,
    {
        self.multi_str.find_map(|x| f(&(self.callback)(x)))
    }
}
...
// TODO it would be great to use IntoIterator instead, and then we can replace
// the array implementations with it
